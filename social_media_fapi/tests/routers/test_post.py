...
from social_media_fapi.tests.helpers import create_comment, create_post, like_post
...
def mock_generate_cute_creature_api(mocker):
    return mocker.patch(
        "social_media_fapi.tasks._generate_cute_creature_api",
...
async def test_get_all_posts_sort_likes(
    ...
    assert response.status_code == 200
    data = response.json()
    expected_order = [1, 2]  # Post 2 has 1 like, Post 1 has 0 likes
    post_ids = [post["id"] for post in data]
    assert post_ids == expected_order
